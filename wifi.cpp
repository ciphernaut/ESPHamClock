...
const BCModeSetting bc_modes[N_BCMODES]{{"CW", 19}, {"RTTY", 22}, {"SSB", 38},
                                        {"AM", 49}, {"WSPR", 3},  {"FT8", 13},
                                        {"FT4", 17}};
...
static NTPServer ntp_list[] = {
    // init times to 0 insures all get tried initially
    {"time.google.com", NTP_TOO_LONG},   {"time.apple.com", NTP_TOO_LONG},
    {"pool.ntp.org", NTP_TOO_LONG},      {"europe.pool.ntp.org", NTP_TOO_LONG},
    {"asia.pool.ntp.org", NTP_TOO_LONG}, {"time.nist.gov", NTP_TOO_LONG},
};
...
/* "reverting" refers to restoring PANE_1 after temporarily forced to show DE or
 * DX weather.
 */
...
/* return absolute difference in two time_t regardless of time_t implementation
 ...
 */
time_t nextWiFiRetry(PlotChoice pc) { return (nextWiFiRetry(plot_names[pc])); }

/* given a plot choice return time of its next update.
 * if choice is in play and rotating use pane rotation time else the given
 * interval.
 */
...
/* return time of next rotation for pane pp or brb_next_update if pp ==
 * PANE_NONE.
 */
time_t nextRotation(PlotPane pp) {
  ...
  for (int i = 0; i < PANE_N; i++)
    if ((isPaneRotating(pp) || isSpecialPaneRotating(pp)) &&
        next_rotation[i] > latest_rot)
...
static void geolocateIP(const char *ip) {
  ...
  if (iploc_client.connect(backend_host, backend_port)) {
    ...
    // expect 4 lines: LAT=, LNG=, IP= and CREDIT=, anything else first line is
    // error message
 ...
 * else search ntp_list for the fastest so far, if all look bad then just roll
 * through. N.B. never return NULL
 */
 ...
 * non-verbose is used for automatic retries that should not clobber the
 * display.
 */
static void initWiFi(bool verbose) {
  ...
  // begin
  // N.B. ESP seems to reconnect much faster if avoid begin() unless creds
  // change N.B. non-RPi UNIX systems return NULL from getWiFI*()
  WiFi.mode(WIFI_STA);
  const char *myssid = getWiFiSSID();
  const char *mypw = getWiFiPW();
  if (myssid && mypw &&
      (strcmp(WiFi.SSID().c_str(), myssid) || strcmp(WiFi.psk().c_str(), mypw)))
  ...
  uint32_t timeout =
      verbose ? 30000UL
              : 3000UL; // dont wait nearly as long for a retry, millis
...
void initSys() {
  ...
  if (useGeoIP() || init_iploc || init_locip) {
    ...
    if (getGPSDLatLong(&ll)) {
      ...
      tftMsg(true, 0, "GPSD: %.2f%c %.2f%c", fabsf(de_ll.lat_d),
             de_ll.lat_d < 0 ? 'S' : 'N', fabsf(de_ll.lng_d),
             de_ll.lng_d < 0 ? 'W' : 'E');
    ...
    if (getNMEALatLong(ll)) {
      ...
      tftMsg(true, 0, "NMEA: %.2f%c %.2f%c", fabsf(de_ll.lat_d),
             de_ll.lat_d < 0 ? 'S' : 'N', fabsf(de_ll.lng_d),
             de_ll.lng_d < 0 ? 'W' : 'E');
  ...
  if (useGPSDTime()) {
    ...
    if (useLocalNTPHost()) {
      ...
      if (getNTPUTC(user_ntp))
        tftMsg(true, 0, "NTP %s: %d ms\r", user_ntp->server,
               user_ntp->rsp_time);
      else
      ...
      for (int i = 0; i < N_NTP; i++) {
        ...
        if (best_ntp &&
            (skip_skip || tft.getChar(NULL, NULL) ||
      ...
      if (best_ntp)
        tftMsg(true, 0, "Best NTP: %s %d ms\r", best_ntp->server,
               best_ntp->rsp_time);
      else
 ...
 * turn on if passes above hi threshold and saw_lo, turn off if passes below lo
 * threshold and saw_hi.
 */
static void doAutoMap(CoreMaps cm, float value_now, float thresh_hi,
                      float thresh_lo) {
...
static void checkAutoMap() {
  ...
  if (retrieveDRAP(drap) && space_wx[SPCWX_DRAP].value_ok)
    doAutoMap(CM_DRAP, space_wx[SPCWX_DRAP].value, DRAP_AUTOMAP_ON,
              DRAP_AUTOMAP_OFF);

  AuroraData a;
  if (retrieveAurora(a) && space_wx[SPCWX_AURORA].value_ok)
    doAutoMap(CM_AURORA, space_wx[SPCWX_AURORA].value, AURORA_AUTOMAP_ON,
              AURORA_AUTOMAP_OFF);
}
...
void checkBGMap(void) {
  ...
  // note whether core_map is one of the BC maps and whether it's time for it to
  // update
...
static bool retrieveBandConditions(char *config) {
  ...
  snprintf(query, sizeof(query),
           "%s?YEAR=%d&MONTH=%d&RXLAT=%.3f&RXLNG=%.3f&TXLAT=%.3f&TXLNG=%.3f&"
           "UTC=%d&PATH=%d&POW=%d&MODE=%d&TOA=%.1f",
           bc_page, year(t), month(t), dx_ll.lat_d, dx_ll.lng_d, de_ll.lat_d,
           de_ll.lng_d, hour(t), show_lp, bc_power, bc_modevalue, bc_toa);

  // build local cache file name
  char cache_fn[100];
  snprintf(cache_fn, sizeof(cache_fn), "bc-%010u.txt",
           stringHash(query)); // N.B. see cleanCache() above
  ...
  if (fp) {
    ...
    // first line is CSV path reliability for the requested time between DX and
    // DE, 9 bands 80-10m
    ...
    for (int r = 0; r < BMTRX_ROWS; r++) {
      ...
      int utc_hr;
      if (sscanf(buf, "%d %f,%*f,%f,%f,%f,%f,%f,%f,%f", &utc_hr, &rel[0],
...
bool setPlotChoice(PlotPane pp, PlotChoice pc) {
  ...
  case PLOT_CH_COUNTDOWN:
    ...
    if (getSWDisplayState() == SWD_NONE) {
      plot_ch[pp] = pc; // must set here so force arg ripples through to
                        // drawCDTimeRemaining()
...
time_t getNTPUTC(NTPServer *ntp) {
  ...
  time_t unix_s = crackBE32(&buf[40]) -
                  2208988800UL;          // packet transmit time - (1970 - 1900)
  ...
  uint16_t transit_time =
      (rx_ms - tx_ms) / 2; // transit = half the round-trip time
...
void sendUserAgent(WiFiClient &client) {
  // don't send full list until first time main page is up to insure all
  // subsystems are up.
  static bool ready;
  if (mainpage_up)
    ready = true;
  ...
  if (logUsageOk() && ready) {
    ...
    case SWD_NONE:
      ...
      case MAPP_ROB:
        ...
        break;
      default:
        fatalError("sendUserAgent() map_proj %d", map_proj);
      }
      break;
    case SWD_MAIN:
      main_page = 2;
    ...
    for (int i = 0; i < PANE_N; i++)
      plotops[i] =
          isPaneRotating((PlotPane)i) ? 100 + (int)plot_ch[i] : (int)plot_ch[i];

    // prefix map style with N unless showing night and/or R if in a rotation
    // set
    ...
    int rr_score =
        (gbl_on ? (has_el ? 2 : 1) : 0) | (rig_on ? 4 : 0) | (flrig ? 8 : 0);
    ...
    // number of dashed colors                      // added to first LV6
    // in 2.90
    int n_dashed = 0;
    ...
    int units =
        showDistKm() ? 1 : (showATMhPa() ? 2 : 0); // only Brits mix miles + hPa

    // panzoom
    bool pz =
        pan_zoom.zoom != MIN_ZOOM || pan_zoom.pan_x != 0 || pan_zoom.pan_y != 0;
    ...
    snprintf(
        ...
        "\r\n",
        platform, hc_version, ESP.getChipId(),
        (long long)getUptime(NULL, NULL, NULL, NULL), crc, map_style, main_page,
        mapgrid_choice, plotops[PANE_1], plotops[PANE_2], plotops[PANE_3],
        de_time_fmt, brb, dx_info_for_sat, rss_code, units, getNBMEConnected(),
        gpio, io, getBMETempCorr(BME_76), getBMEPresCorr(BME_76), desrss,
        dxsrss, BUILD_W, dpy_mode,

        // new for LV5:
        alarms, getCenterLng(), (int)auxtime /* getDoy() before 2.80 */,
        names_on, getDemoMode(), (int)getSWEngineState(NULL, NULL),
        (int)getBigClockBits(), utcOffset(), gpsd, rss_interval, dayf, rr_score,

        // new for LV6:
        useMagBearing(), n_dashed, ntp, path, spots, call_fg, call_bg,
        !clockTimeOk(), // default clock 0 == ok

        // new for LV7:
        scrollTopToBottom(),
        0, // nMoreSroll rm V4.04
        1, // rankSpaceWx rm V4.07
        showNewDXDEWx(), getPaneRotationPeriod(), pw_file != NULL, n_roweb > 0,
        pz, plotops[PANE_0], screenIsLocked(), showPIP(), (int)getGrayDisplay(),
        wl, aup_hr, 0);

  } else {
    snprintf(ua, sizeof(ua), "User-Agent: %s/%s (id %u up %lld) crc %d\r\n",
             platform, hc_version, ESP.getChipId(),
             (long long)getUptime(NULL, NULL, NULL, NULL), flash_crc_ok);
  }
...
/* skip the given wifi client stream ahead to just after the first blank line,
 * return whether ok. this is often used so subsequent stop() on client doesn't
 * slam door in client's face with RST. Along the way, if find a header field
 * with the given name (unless NULL) return value in the given string. if header
 * is not found, we still return true but value[0] will be '\0'.
 */
bool httpSkipHeader(WiFiClient &client, const char *header, char *value,
                    int value_len) {
  char line[200];
...
bool httpSkipHeader(WiFiClient &client) {
  return (httpSkipHeader(client, "Remote_Addr: ", remote_addr,
                         sizeof(remote_addr)));
}
...
static bool updateDRAP(const SBox &box) {
  ...
  if (ok) {
    ...
    if (!drap.data_ok) {
      ...
      // plot
      plotXY(box, drap.x, drap.y, DRAPDATA_NPTS, "Hours", "DRAP, max MHz",
             DRAPPLOT_COLOR, 0, 0, drap.y[DRAPDATA_NPTS - 1]);
    }
...
static bool updateKp(const SBox &box) {
  // data are provided every 3 hours == 8/day. collect 7 days of history + 2
  // days of predictions
  KpData kp;
  ...
  if (ok) {
    ...
    if (!kp.data_ok) {
      ...
      // plot
      plotXY(box, kp.x, kp.p, KP_NV, "Days", "Planetary Kp", KP_COLOR, 0, 9,
...
static bool updateDST(const SBox &box) {
  ...
  if (ok) {
    ...
    if (!dst.data_ok) {
      ...
      // plot
      plotXY(box, dst.age_hrs, dst.values, DST_NV, "Hours",
             "Dist Storm Time, nT", DST_COLOR, 0, 0, space_wx[SPCWX_DST].value);
    }
...
static bool updateXRay(const SBox &box) {
  ...
  if (ok) {
    ...
    if (!xray.data_ok) {
      ...
      char level_str[10];
      plotXYstr(box, xray.x, xray.l, XRAY_NV, "Hours", "GOES 16 X-Ray",
                XRAY_LCOLOR, -9, -2, NULL) &&
          plotXYstr(box, xray.x, xray.s, XRAY_NV, NULL, NULL, XRAY_SCOLOR, -9,
                    -2, xrayLevel(level_str, space_wx[SPCWX_XRAY]));
    }
...
static bool updateSunSpots(const SBox &box) {
  ...
  if (ok) {
    ...
    if (!ssn.data_ok) {
      ...
      plotXYstr(box, ssn.x, ssn.ssn, SSN_NV, "Days", "Sunspot Number",
                SSN_COLOR, 0, 0, label);
    }
...
/* retrieve and plot latest and predicted solar flux indices, return whether io
 * ok.
 */
...
/* retrieve and plot latest and predicted solar wind indices, return whether io
 * ok.
 */
static bool updateSolarWind(const SBox &box) {
  ...
  if (ok) {
    if (!sw.data_ok) {
      ...
      plotXY(box, sw.x, sw.y, sw.n_values, "Hours", "Solar wind", SWIND_COLOR,
             0, 0, sw.y[sw.n_values - 1]);
    }
...
static bool updateBzBt(const SBox &box) {
  ...
  if (ok) {
    ...
    if (!bzbt.data_ok) {
      ...
      // plot
      char bz_label[30];
      snprintf(bz_label, sizeof(bz_label), "%.1f",
               bzbt.bz[BZBT_NV - 1]); // newest Bz
      plotXYstr(box, bzbt.x + f25, bzbt.bz + f25, BZBT_NV - f25, "Hours",
                "Solar Bz and Bt, nT", BZBT_BZCOLOR, min_bzbt, max_bzbt,
                NULL) &&
...
static bool updateBandConditions(const SBox &box, bool force) {
  ...
  bool update_bc =
      force || (CM_PMACTIVE() && tdiff(bc_time, map_time) >= 3600) ||
      (tdiff(nowWO(), bc_time) >= 3600) || myNow() >= bc_matrix.next_update;
  ...
  if (bc_matrix.ok) {
    ...
    // if problem persists more than an hour, this prevents the tdiff's above
    // from being true every time
...
static bool updateAurora(const SBox &box) {
  ...
  if (ok) {
    ...
    if (!a.data_ok) {
      ...
      snprintf(aurora_label, sizeof(aurora_label), "%.0f",
               a.percent[a.n_points - 1]);
      plotXYstr(box, a.age_hrs, a.percent, a.n_points, "Hours",
                "Aurora Chances, max %", AURORA_COLOR, 0, 100, aurora_label);
    }
...
bool checkBCTouch(const SCoord &s, const SBox &b) {
  ...
  if (inBox(s, power_b)) {
    ...
    SBox ok_b;
    MenuInfo menu = {menu_b, ok_b,           UF_CLOCKSOK, M_NOCANCEL,
                     1,      NARRAY(mitems), mitems};
    ...
    // always redo BC if nothing else to erase menu but only update voacap if
    // power changed
    ...
    for (int i = 0; i < N_BCMODES; i++)
      mitems[i] = {
          MENU_1OFN, bc_modevalue == bc_modes[i].value, 1, 5, bc_modes[i].name,
          0};
...
void updateWiFi(void) {
  ...
  for (int i = PANE_0; i < PANE_N; i++) {
    ...
    if (t0 >= next_rotation[pp] &&
        (isPaneRotating(pp) || isSpecialPaneRotating(pp))) {
      ...
      if (pc == PLOT_CH_DXCLUSTER)
        fresh_redraw[PLOT_CH_DXCLUSTER] = true;
      if (pc == PLOT_CH_ADIF)
        fresh_redraw[PLOT_CH_ADIF] = true;
      if (pc == PLOT_CH_ONTA)
        fresh_redraw[PLOT_CH_ONTA] = true;
      if (pc == PLOT_CH_CONTESTS)
        fresh_redraw[PLOT_CH_CONTESTS] = true;
      if (pc == PLOT_CH_DXPEDS)
        fresh_redraw[PLOT_CH_DXPEDS] = true;
...
/* get next line from client in line[] then return true, else nothing and return
 * false. line[] will have \r and \n removed and end with \0, optional line
 * length in *ll will not include \0. if line is longer than line_len it will be
 * silently truncated.
 */
bool getTCPLine(WiFiClient &client, char line[], uint16_t line_len,
                uint16_t *ll) {
...
void initWiFiRetry() {
  ...
  memset(fresh_redraw, 1,
         sizeof(fresh_redraw)); // works ok even if bools aren't bytes
...
/* show PLOT_CH_DEWX or PLOT_CH_DXWX in pp immediately then arrange to resume
 * normal pp operation after DXPATH_LINGER using revert_time and revert_pp.
 */
static void revertWXPane(PlotPane pp, PlotChoice wxpc) {
  // easier to just show weather immediately without using the pane rotation
  // system
...
/* request that the given pane use fresh data, if and when it next becomes
 * visible. for most panes: if the pane is currently visible: fresh update
 * immediately; if in rotation but not visible:   it is marked for fresh update
 * when its turn comes; if not selected anywhere:         we do nothing. for
 * PLOT_CH_DEWX or PLOT_CH_DXWX which are only for temporary display called
 * reverting: if the pane is currently visible: it will refresh immediately; if
 * in rotation but not visible:   immediately displayed in its pane, normal
 * rotation after DXPATH_LINGER if not selected anywhere:         immediately
 * displayed in PANE_1, normal rotation after DXPATH_LINGER
 */
 ...
 */
void scheduleFreshMap(void) { next_map = 0; }
 ...
 */
time_t nextPaneRotation(PlotPane pp) { return (next_rotation[pp]); }

/* force the given pane to rotate now.
 * it's ok if pp is not actually rotating.
 */
void forcePaneRotation(PlotPane pp) { next_rotation[pp] = 0; }

/* return pane for which taps are to be ignored because a revert is in progress,
 * if any
 */
